...
namespace OdysseyPatch.ShuttleFood
{
    ...
    public static class Patch_JobGiver_SatisfyChemicalNeed_TryGiveJob
    {
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator il)
        {
            ...
            instructionsList.InsertRange(index - 3, new[]
            {
                ...
                new CodeInstruction(OpCodes.Call, typeof(Patch_JobGiver_SatisfyChemicalNeed_TryGiveJob).Method(nameof(GetTakeFromShuttleJob))),
    ...
    public static class Patch_JobGiver_SatisfyChemicalNeed_FindDrugFor
    {
        public static void Postfix(Pawn pawn, Need_Chemical need, ref Thing __result)
        {
            if (Settings.ShuttleFood.Enabled() && __result == null)
            {
                if (pawn.IsColonist && pawn.Map != null)
                {
                    ...
                    foreach (Building_PassengerShuttle shuttle in pawn.Map.GetShuttlesWithGrabbingEnabled())
                    {
                        ...
                        {
                            if (DrugValidator(pawn, addictionHediff, thing) && !shuttle.IsForbidden(pawn) && pawn.CanReach(shuttle, PathEndMode.InteractionCell, Danger.Some))
                            {
                                __result = thing;
                                return;
                            }
        ...
        private static bool DrugValidator(Pawn pawn, Hediff_Addiction addiction, Thing drug) => (bool)typeof(JobGiver_SatisfyChemicalNeed).Method("DrugValidator").Invoke(null, new object[] { pawn, addiction, drug });
    }
}
